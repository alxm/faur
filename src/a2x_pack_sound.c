...
void a_sound__uninit(void)
{
    if(a_settings_getBool("sound.on")) {
        ...
        A_LIST_ITERATE(musicList, AMusic, m) {
            a_music__free(m);
        }
...
{
    if(a_settings_getBool("sound.on")) {
        AMusic* m = a_sdl__music_load(path);
