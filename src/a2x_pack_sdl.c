...
{
    A_LIST_ITERATE(g_buttons->list, ASdlPhysicalInput*, b) {
        free(b->name);
    }

    A_LIST_ITERATE(g_analogs->list, ASdlPhysicalInput*, a) {
        free(a->name);
    }

    A_LIST_ITERATE(g_touchScreens->list, ASdlPhysicalInput*, t) {
        free(t->name);
    }
